		...
		if (state.repos.length > 1) {
			step = this.createConfirmStep(appendReposToTitle(`Confirm ${context.title}`, state, context), [
				...
				FlagsQuickPickItem.create<Flags>(state.flags, ['--rebase'], {
					...
					description: '--rebase',
					detail: `将使用变基方式拉取 ${state.repos.length} 个仓库`,
				}),
			]);
				...
				if (branch?.upstream == null) {
					...
					step = this.createConfirmStep(appendReposToTitle(`Confirm ${context.title}`, state, context), [
						FlagsQuickPickItem.create<Flags>(state.flags, [], {
							...
							detail: `将拉取${
								branch.state.behind
									? ` ${pluralize('commit', branch.state.behind)} 到 ${GitReference.toString(
											branch,
									  )}`
			...
			const pullDetails =
				status?.state.behind != null
					? ` ${pluralize('commit', status.state.behind)} 到 $(repo) ${repo.formattedName}`
