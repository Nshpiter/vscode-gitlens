...
export class FetchGitCommand extends QuickCommand<State> {
	constructor(container: Container, args?: FetchGitCommandArgs) {
		super(container, 'fetch', 'fetch', '拉取', { description: '从一个或多个远程仓库拉取更改' });
			...
			step = this.createConfirmStep(
				...
				[
					FlagsQuickPickItem.create<Flags>(state.flags, [], {
						label: this.title,
						detail: `将拉取 ${GitReference.toString(state.reference)}`,
					}),
				],
			);
		} else {
